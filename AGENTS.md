...
- Output helpers: `cecho "color" "msg"`, `decho "color" "msg"` (verbose-only), `aecho [-s] ARRAY "prefix" "color" "pfx_color"` (optional `-s` sorts items alphabetically); interactive helpers: `read_key` (single keypress → `up/down/left/right/space/enter/esc/eof` or literal char), `read_yes_no "prompt" "default"` (sets `REPLY_YN`; keypress on TTY, typed fallback otherwise)

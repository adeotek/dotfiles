...
| D6 | `.stow-local-ignore` | medium | dead-code | GNU Stow only reads `.stow-local-ignore` from **inside each package dir** — the repo-root file is never consulted (verified with stow `-v5`); its entries are also redundant with the built-in defaults. | Delete the file | fix | done |

...
The templates are in `<dotfiles>/hermes/.config/systemd/user/`. They are
host-specific — absolute paths under `/home/toor`, as generated by
`hermes gateway install` — so adapt the paths when provisioning another
host. (The `hermes-update*` units use the portable `%h` specifier.)

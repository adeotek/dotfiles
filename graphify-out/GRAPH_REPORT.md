...
Cohesion: 0.10
Nodes (10): Shell Prompt Customization Tool, base-tools-install.sh script, glow-install.sh script, nvim-setup.sh script, oh-my-posh-setup.sh script, rtk-install.sh script, rtk-setup.sh script, starship-install.sh script (+2 more)
...
Cohesion: 0.14
Nodes (10): AI Coding Tools (claude-code + opencode), Claude Code Plugin Marketplace, claude-code-install.sh script, CLAUDECODE_PLUGINS, claude-code-setup.sh script, opencode-install.sh script, copy_skills_if_missing(), opencode-setup.sh script (+2 more)
...
Cohesion: 0.14
Nodes (8): Multi-Distro Support (Arch/Debian/Fedora), OS-Dispatch Pattern (case $CURRENT_OS_ID), stow_package() Helper Function, aecho(), get_stow_command(), get_vv(), read_key(), read_yes_no()
...
### Community 118 - "kitty-install.sh"
Cohesion: 0.32
Nodes (4): Install-Then-Setup Pattern, Terminal Emulator, kitty-setup.sh script, tabby-setup.sh script

### Community 119 - "copy_files_if_missing"
Cohesion: 0.40
Nodes (3): copy_files_if_missing(), hermes-install.sh script, hermes-setup.sh script

## Knowledge Gaps
- **135 isolated node(s):** `$schema`, `plugin`, `_options.sh script`, `MINIMAL_TASKS`, `CONSOLE_ONLY_TASKS` (+130 more)
...
- **Why does `cecho()` connect `cecho` to `homebrew-install.sh`, `yazi-install.sh`, `docker-install.sh script`, `claude-code-setup.sh`, `_helpers.sh`, `uv-install.sh`, `herdr-setup.sh`, `k8s-repo-install.sh`, `golang-install.sh`, `copy_files_if_missing`, `mise-install.sh`?**
...
- **Why does `stow_package()` connect `cecho` to `homebrew-install.sh`, `yazi-install.sh`, `ghostty-install.sh`, `_helpers.sh`, `kitty-install.sh`, `fastfetch-install.sh`?**
